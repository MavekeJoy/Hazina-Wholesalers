...
function Reporting() {
  ...
  // ── AI Insights — calls backend, not Anthropic directly ──────────────────
  const handleGetAIInsights = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/reports/ai-insights`, {
        method: "POST",
        headers: {
                ...
                <>
                  <ResponsiveContainer width="100%" height={400}>
                    <AreaChart data={reportData.revenue}>
                      ...
                      <Tooltip formatter={(v) => `Ksh ${Number(v).toLocaleString()}`} />
                      <Legend />
                      <Area type="monotone" dataKey="revenue" stroke="#16a34a"
                        fillOpacity={1} fill="url(#colorRevenue)" name="Revenue (Ksh)" />
                    </AreaChart>
                  </ResponsiveContainer>
              ...
              <div className="bg-white rounded-xl shadow-md p-8">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-green-600 text-white">
                      <tr>
                        {["Product", "SKU", "Category", "Qty", "Min", "Price", "Value", "Status"].map((h) => (
